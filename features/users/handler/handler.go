...
func (uh *UserHandler) AddUser() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		serviceInput.IP = userIP
		serviceInput.UserAgent = userAgent

		result, err := uh.s.AddUser(*serviceInput)
		...
		return c.JSON(http.StatusCreated, helper.FormatResponse(true, "add user success", result))
	}
}

func (uh *UserHandler) LoginAdmin() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		result, err := uh.s.LoginAdmin(input.Email, input.Password, userIP, userAgent)
...
func (uh *UserHandler) ForgotPassword() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		result := uh.s.ForgotPassword(input.Email)

		if result != nil {
			fmt.Println("This is the error: ", result)
			return c.JSON(http.StatusInternalServerError, helper.FormatResponse(false, result.Error(), nil))
		}
...
func (uh *UserHandler) ResetPassword() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		result := uh.s.ResetPassword(token, input.Email, input.Password)
